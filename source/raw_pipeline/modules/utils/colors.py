...
_RGB_TO_YUV = torch.Tensor([[0.299, 0.587, 0.114],
...
def rgb2yuv(rgb):
    ...
    if clip_rgb:
        rgb = torch.clamp(rgb, 0, 1)
    return _mul(_RGB_TO_YUV, rgb)


def yuv2rgb(rgb):
    """YCbCr to sRGB conversion."""
    clip_rgb = False
    rgb = _mul(torch.inverse(_RGB_TO_YUV), rgb)
    if clip_rgb:
        rgb = torch.clamp(rgb, 0, 1)
    return rgb
...
def lab2xyz(lab, illuminant="D65", observer="2"):
    ...
    if lab.is_cuda:
        xyz_ref_white = xyz_ref_white.cuda()
    xyz_ref_white = xyz_ref_white.unsqueeze(2).unsqueeze(
        2).repeat(1, 1, out.shape[2], out.shape[3])
