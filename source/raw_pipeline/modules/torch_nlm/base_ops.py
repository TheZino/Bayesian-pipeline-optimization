...
def get_gaussian_kernel(kernel_size: int = 5, sigma: float = 1., ndim: int = 2) -> np.ndarray:
    ...
    if ndim == 1:
        ...
        kernel = gauss[None, None, :] * \
            gauss[None, :, None] * gauss[:, None, None]
...
def apply_gaussian_filter(X: torch.Tensor,
    ...
    gaussian_kernel = torch.cat(
        [gaussian_kernel for _ in range(n_channels)], 0)
    gaussian_kernel = torch.cat(
        [gaussian_kernel for _ in range(n_channels)], 1)
    if ndim == 2:
...
def apply_mean_filter(X: torch.Tensor,
    ...
    filter = filter.repeat(1, X.shape[1], 1, 1)
    pad = kernel_size // 2
...
def non_local_means_loop_index(X: torch.Tensor,
    ...
    def calculate_weights_2d(X: torch.Tensor,
        ...
        neighbours = X[:, :, idxs[0], idxs[1]].reshape(
            X.shape[0], n, X.shape[2], X.shape[3])
        ...
        for i in range_h:
            for j in range_w:
                ...
                if len(all_idxs[0]) >= sub_filter_size:
                    weights, neighbours = calculate_weights_2d(
                        X, all_idxs, std_2)
        ...
        for i in range_h:
            for j in range_w:
                for k in range_d:
                    ...
                    if len(all_idxs[0]) >= sub_filter_size:
                        weights, neighbours = calculate_weights_3d(
                            X, all_idxs, std_2)
                        weights_sum += torch.sum(weights, 1)
                        output += weights.multiply(neighbours).sum(1)
                        all_idxs = [[], [], []]
